...
- Viridis colormap
- Displays target score per slice/channel
"""
...
class TrainingWidget(QWidget):
    ...
    def __init__(self, viewer):
        ...
        self.target_score_label = QLabel("Target score: N/A")
        layout.addWidget(self.target_score_label)

        self.targets = None  # Will hold the loaded targets.npy
    ...
    def _refresh_layers(self, event=None):
        ...
        for layer in self.viewer.layers:
            if isinstance(layer, napari.layers.Image):
                ...
                # Attach slice/channel change callback
                layer.events.current_step.connect(self._update_target_score)
                layer.events.selected_channel.connect(self._update_target_score)
        index = self.image_combo.findText(current)
        if index >= 0:
            self.image_combo.setCurrentIndex(index)
        elif self.image_combo.count() > 0:
            self.image_combo.setCurrentIndex(0)
        self._update_target_score()
    ...
    def _pick_target_file(self):
        ...
        if path:
            ...
            # Load targets for real-time display
            try:
                self.targets = np.load(path)
            except Exception as e:
                self.message_label.setText(f"Failed to load targets: {e}")

    def _update_target_score(self, event=None):
        """Update target score based on slice and channel"""
        if self.targets is None or self.image_combo.count() == 0:
            self.target_score_label.setText("Target score: N/A")
            return

        layer_name = self.image_combo.currentText()
        layer = self.viewer.layers[layer_name]

        # Slice index (image index)
        slice_idx = int(layer.current_step) if layer.current_step is not None else 0
        slice_idx = min(max(slice_idx, 0), len(self.targets)-1)

        # Channel index
        channel_idx = int(layer.selected_channel) if hasattr(layer, 'selected_channel') and layer.selected_channel is not None else 0

        # Display target
        score = self.targets[slice_idx]  # Replace if per-channel mapping exists
        self.target_score_label.setText(f"Target score (slice {slice_idx}, channel {channel_idx}): {score}")
